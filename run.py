...
from utils import (config_parser, find_POI, img2mse, load_blender,
                   load_llff_data, show_img)
...
def run():
    ...
    if delta_brightness != 0:
        ...
        # obs_img= (np.array(obs_img) * 255).astype(np.uint8)
        # brightness_path=os.path.join("./brightness")
        # show_img("Observed_image.png", obs_img)
    ...
    testsavedir = os.path.join(output_dir, model_name,str(args.delta_phi)+'_'+\
                str(args.delta_theta)+'_'+str(args.delta_psi)+'_'+str(args.delta_t))
