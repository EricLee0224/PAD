...
from func import denormalization
from metric import *
import numpy as np
...
def save_fig(test_img, recon_imgs, scores, gts):
    num = len(scores)
    vmax = scores.max() * 255.
    vmin = scores.min() * 255.
    for i in range(num):
        ...
        os.makedirs(os.path.join(save_dir,class_name+'_'+str(i)),exist_ok=True)
        imageio.imwrite(os.path.join(save_dir,class_name+'_'+str(i),"ref.png"),img)
        imageio.imwrite(os.path.join(save_dir,class_name+'_'+str(i),"rgb8.png"),recon_img)
        imageio.imwrite(os.path.join(save_dir,class_name+'_'+str(i),"gt.png"),gt)
        ...
        fig_img.savefig(os.path.join(save_dir,class_name+'_'+str(i),"heatmap.png"), dpi=300,bbox_inches='tight', pad_inches = -0.1)
        plt.close()
...
def compare_feature(ref_feature,rgb_feature):
    loss=(ref_feature-rgb_feature)**2 #(272,224,224)
    result_sum=torch.mean(loss,axis=0) #(224,224)
    result_sum = gaussian_filter(result_sum, sigma=4)
    # result=(result_sum-result_sum.min())/(result_sum.max()-result_sum.min())
    return result_sum
    # result=result_sum.squeeze(0)

parser = argparse.ArgumentParser(description='Testing')
parser.add_argument('--obj', type=str, default='98')
parser.add_argument('--data_type', type=str, default='mvtec')
parser.add_argument('--dataset_path', type=str, default='./data/MLAD-Sim')
...
# obs_img=imageio.imread(obs_img_dir)
# rgb=imageio.imread(rgb_dir)
...
output_dir=os.path.join(output_path,class_name,'loftr')
...
# save_fig(test_imgs,recon_imgs,scores,gt_mask_list)