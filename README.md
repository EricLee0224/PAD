# PAD (NeurlPS'23 D&B Track, UnderReview): Official Project Repository   
...
- We introduced \textbf{P}ose-agnostic \textbf{A}nomaly \textbf{D}etection (PAD), a novel setting for object anomaly detection from diverse viewpoints/poses, breaking free from the constraints of stereotypical pose alignment, and taking step forward to pratical anomaly detection and localization tasks.    
- We developed the \textbf{M}ulti-pose \textbf{A}nomaly \textbf{D}etection (MAD) dataset, the first attempt at evaluating the pose-agnostic anomaly detection. Our dataset comprises MAD-Sim and MAD-Real, containing 11,000+ high-resolution RGB images of multi-pose views from 20 diverse shapes and colors of LEGO toys, offering pixel-precise ground truth for 3 types of anomalies.   
